...
async function migrate() {
  ...
  try {
    ...
    await db.execute(sql`ALTER TABLE "workspace" ADD COLUMN IF NOT EXISTS "is_pro" boolean DEFAULT false NOT NULL;`);
    ...
    try {
      await db.execute(sql`CREATE UNIQUE INDEX IF NOT EXISTS "project_collection_slug_unique" ON "project" ("collection_slug");`);
