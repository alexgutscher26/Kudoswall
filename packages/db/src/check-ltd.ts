...
async function main() {
  ...
  console.log(`Organization Table: ${orgs.length > 0 ? orgs.map(o => `Found (ID: ${o.id}, Name: ${o.name}, Plan: ${o.plan})`).join(", ") : "Not Found"}`);

  // Check Workspace
  const wss = await db.query.workspace.findMany({
    where: or(eq(workspace.id, targetId), eq(workspace.ownerId, targetId))
  });
  console.log(`Workspace Table: ${wss.length > 0 ? wss.map(w => `Found (ID: ${w.id}, Name: ${w.name}, Plan: ${w.plan})`).join(", ") : "Not Found"}`);
