import CollectionWizard from "@/app/[workspaceSlug]/[projectSlug]/collection-wizard";
...
interface CollectionPageViewProps {
  project: any; // Using any for simplicity as it's a shared component, prefer strict types usually
}

export default function CollectionPageView({ project }: CollectionPageViewProps) {
  const projectName = project.name || "KudosWall";
      ...
      <main className="mx-auto flex w-full max-w-2xl grow flex-col items-center px-4 pt-24 pb-12 sm:px-6">
        <div className="w-full">
          <ErrorBoundary name="Collection Wizard">
            <CollectionWizard project={project} />
          </ErrorBoundary>
        </div>
      </main>
